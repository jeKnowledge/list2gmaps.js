var list2gmaps = (function () {
    ...
    var dispMarker = function (markers, state) {
        $.each(markers, function (index, marker) {
            ...
            $('#' + marker.getTitle().replace(/ /g, "-").replace(/\(/g, "").replace(/\)/g, "")).toggle(state && marker.onMap);
        });
    };
    ...
    var addInfo = function (selector, information, id) {
        $('#' + selector).append('<div " id=' + id.replace(/ /g, "-").replace(/\(/g, "").replace(/\)/g, "") + ' class="list2gmaps-info-container">' + information + '</div>');
    };
    ...
    var addEventMap = function () {
        google.maps.event.addListener(map, 'bounds_changed', function() {
            $.each(markers, function (index, marker) {
                $('#' + marker.getTitle().replace(/ /g, "-").replace(/\(/g, "").replace(/\)/g, "")).toggle(marker.getVisible() && map.getBounds().contains(marker.getPosition()));
                marker.onMap = map.getBounds().contains(marker.getPosition());
            });
        });
    };
